...
screen = pygame.display.set_mode((800, 800))  # Creates the game screen
...
for i in range(10000):  # Loop a bunch of times
    direction = random.choice(['x', 'y'])  # Cardinal directions

    if direction == 'x':
        xRoll = random.randrange(1, 100)  # Fewer turns
        if xRoll == 1:
            xdir *= -1
        xpos += xdir
    else:
        yRoll = random.randrange(1, 100)  # Fewer turns
        if yRoll == 1:
            ydir *= -1
        ypos += ydir
